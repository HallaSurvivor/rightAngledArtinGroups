...
\begin{abstract}
  ...
  The raag functor $A$ is an equivalence between the category of graphs 
  and the subcategory of groups equipped with a bonus coalgebra structure.
  This additionally provides a categorical justification for the many 
  disparate results relating the combinatorics of $\Gamma$ to the algebra 
  of $A \Gamma$.
\end{abstract}
...
\label{intro}
  ...
  TODO: say that roman letters will always be in $\mathsf{Grp}$ and 
  greek letters will be in $\mathsf{Gph}$.
  ...
  abelian groups. If $\square$ is the graph with $4$ vertices $a,b,c,d$ and 
  ...
  \cite{kimGeometryCurveGraph2014}. These connections proved useful in 
  Agol's famed resolution of the Virtual Haken Conjecture (TODO: cite this),
  see this survey, for instance \cite{yamashitaVeryBriefIntroduction2013}.
  (TODO: this citation doesn't have an attached URL)

  Interestingly, the combinatorial structure of $\Gamma$ is closely related to the 
  ...
  (TODO: fact check this BNS invariant thing)
  ...
  (TODO: rewrite this to phrase things as an equivalence of categories?)

  \begin{thm}
      A group $G$ is isomorphic to a raag $A\Gamma$
      if and only if there is a ``structure map''
      $\mathfrak{g} : G \to ACG$ making a pair of diagrams
      commute (see figure \ref{fig:coalg}). 

      Moreover, if $(G,\mathfrak{g})$ and $(H,\mathfrak{h})$ are groups with structure 
      maps, then a homomorphism $f : G \to H$ is of the form $A \varphi$ for 
      some $\varphi$ a map of graphs if and 
      only if it respects the structure maps in the sense that the 
      ...
      \[
          ...
          \arrow["{\mathfrak{g}}"{description}, from=1-1, to=3-1]
          \arrow["{\mathfrak{h}}"{description}, from=1-3, to=3-3]
          \end{tikzcd}
      \]
  \end{thm}
...
First we need structure maps for $A\Gamma$ and $A \Delta$. For $A \Gamma$
this is a map $\mathfrak{g} : A \Gamma \to ACA \Gamma$.
...
(TODO: probably put the definition sooner, lol)
...
which clearly commutes. This tells us that $f$ comes from a graph homomorphism,
as indeed it did!
...
underlying graphs\footnote{Though we will see later that we can effectively 
recover the underlying graph from the coalgebra structure}! 
(TODO: again, here we're kind of using the fact that $G$ has a coalgebra 
structure iff it's $A \Gamma$ for some graph\ldots maybe we should mention 
this explicitly somewhere?)
...
(TODO: find a citation for this? It's definitely in borceux or categories work.)

In our special case, $\eta_\Gamma : \Gamma \to CA\Gamma$ is the map sending 
...
\begin{align*}
  ...
  \mathsf{Grp} \\
  \Gamma 
  &\mapsto (A\Gamma, \eta_\Gamma) 
  &\mapsto A \Gamma
\end{align*}

TODO: fix this formatting. Maybe just use q.uiver
...
Again, we emphasize that, given the coalgebra structure $\mathfrak{g}$ this 
process is effective. In section \ref{computing} we show that we can recover 
the underlying graph from $\mathfrak{g}$ and how to recover the underlying 
graph map from a cohomomorphism.
...
\cite{dromsIsomorphismsGraphGroups1987}, and sharpened by 
TODO: cite the rational cohomology paper.
...
in the sense of TODO: cite joy of cats}, and thus has all equalizers. 
...
$\mathfrak{g} : G \to ACG$ and $\mathfrak{h} : H \to ACH$. Indeed this is the case.
...
(TODO: Actually I think this is a theorem\ldots Double check this)
