...
\begin{abstract}
  ...
  The raag functor $A$ is half of an equivalence between the category
  of graphs and graph homomorphisms and the category of
  groups admitting a bonus coalgebra structure with homomorphisms preserving 
  this coalgebra structure. This moreover provides a categorical justification
  for the close relationship between a graph and its raag.
\end{abstract}
...
\label{intro}
  ...
  \cite{kimGeometryCurveGraph2014a}.
  These groups are incredibly complicated in general (for instance, 
  TODO: isn't there a raag which has thompson's group F as a finite 
  index subgroup? Find a citation.), and by relating
  ...
  from the information in $\Gamma$ \cite{meierBieriNeumannStrebelInvariantsGraph1995}.
  ...
  certain \important{descent data} in the form of a 
  \important{coalgebra structure}, and group homomorphisms which are moreover
  cohomomorphisms.
  ...
  giudiciDiametersCommutingGraphs2010a,
  ...
  \cite{neumannProblemPaulErdos1976a}.
  ...
  readers, so in section \ref{refiew} we will briefly review the machinery 
...
Importantly, this same approach works even if we merely know $G$ and $H$ 
are isomorphic to raags because they have coalgebra structures 
$\mathfrak{g}$ and $\mathfrak{h}$. We don't need to know the underlying graphs!
...
bojanczykRecognisableLanguagesMonads2015a,
...
depaivaDialecticaComonads2021a,
